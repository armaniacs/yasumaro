...
`autonomous-task-closer` セッションで発見した Firefox 動作不能の根本原因（`chrome.offscreen` 依存）と、VFS プローブによる検証結果（`0914b` ブランチ・Firefox 155 で wa-sqlite OPFS SAH / FTS5 / 永続化 / IDB fallback 全 green）を受け、Firefox 対応を3件の PBI に分解したもの。目標バージョンは v6.9.0（奇数マイナー = 新機能ライン）。

## 前提資産

- **VFS プローブ**: `0914b` ブランチ（`testDir/e2e/firefox-opfs-probe.spec.ts` + probe Worker）。未マージ — PBI 10 で main へ取り込む
- **検証済み知見**:
...
| 2 | 2026-09-14-10-test-firefox-e2e-ci.md | **32.0** | 20 / 1 / 80% / 0.5人日 | 回帰網の確保。**09 に依存**（gecko.id と動くビルドが前提）+ 0914b プローブ取り込み |
