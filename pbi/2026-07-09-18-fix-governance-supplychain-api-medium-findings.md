...
## 受け入れ基準
- [x] M27: `@subframe7536/sqlite-wasm`のバージョンがpackage.jsonで固定され、CIにnpm auditの定期実行が追加されている（2026-07-12実装: `1.3.1`にピン留め、`.github/workflows/security-audit.yml`を新設し毎週月曜+手動実行でnpm audit --audit-level=highを実行。既存devDependencies由来の脆弱性8件は別スコープとして残置）
...
- [x] M31: `dashboard.css:348`のmax-width制限がデータ集約パネル専用のクラス/セレクタで個別設定されている（2026-07-12実装済み、コミット66d5d4b）
- [x] M32: `sqlite.ts:327-328`のWALモード設定がスキーマ実行の直前に移動されている（2026-07-11実態調査で確認）
- [x] M33: `sqlite.ts:443-450`のプリペアドステートメントキャッシュがLRU戦略に置き換えられている（2026-07-12実装: `src/offscreen/lruCache.ts`を新設しMapのget時再挿入でLRU化、単体テスト6件追加）
...
## Definition of Done
- [x] 全BDDシナリオが自動テストとして実装されパスする（M29/M30/M32は既存実装確認、M33は新規単体テスト、M27はCI設定のためワークフロー導入をもって代替）
- [x] テストカバレッジが基準を満たす（M33: lruCache.test.ts 6件。M27はCIワークフロー検証が中心）
- [ ] コードレビュー完了
- [x] リファクタリング完了（グリーン後）
- [x] ドキュメント更新済み（M28: package.jsonコメント+専用チェックスクリプト既存確認）
