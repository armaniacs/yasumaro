...
## 受け入れ基準
- [x] M20: `entrypoints/options/index.html:2` のlang属性が固定"en"から変更されている（2026-07-12実装済み、コミット66d5d4b）
- [x] M21: `entrypoints/popup/index.html:769-783` 付近のモーダルが`<dialog>`要素ベースに統一されている（2026-07-12完了: 全5モーダル（`confirmationModal`/`importConfirmModal`/`passwordModal`/`passwordAuthModal`/`privacyConsentModal`）を`<div>`+`focusTrapManager`から`<dialog>`+ネイティブ`showModal()`/`close()`に移行。`privacyConsentModal`のみ「ESCで閉じない」仕様を`cancel`イベントの`preventDefault()`で再現。既存テスト8ファイルを`<dialog>`前提に更新、全体スイート6987件通過）
- [x] M22: `public/_locales/en/messages.json` 内の該当キーが`confirmImport`にリネームされ、対応する日本語キーも整合している（2026-07-12実装済み、コミット07b3747）
- [x] M23: README.mdの未リリースバージョン表記が削除されている（2026-07-11実態調査で確認）
- [x] M24: `wxt.config.ts` のCSP文字列が共通のドメイン定数配列から生成されるようになっている（2026-07-12実装: `src/utils/cspDomains.ts`を新設し`AI_PROVIDER_HOST_PERMISSIONS`/`OPTIONAL_AI_PROVIDER_HOST_PERMISSIONS`定数と`buildConnectSrcDomains()`を定義。`wxt.config.ts`の`host_permissions`/`optional_host_permissions`/CSP文字列すべてがこの単一ソースから生成される。実ビルドで元のmanifest.jsonと文字列完全一致を確認。単体テスト4件追加）
- [x] M25: ESLintまたはBiomeが導入され、`noUnusedLocals: true`相当のルールが有効化されている。`package.json`の`lint`スクリプトが更新されている（2026-07-12実装済み、コミットc139d1e）
- [x] M26: `.github/workflows/release.yml` にビルド後検証（バンドルサイズチェックまたはロードテスト）が追加されている（2026-07-12実装: `scripts/check-bundle-size.mjs`を新設し、Chrome向けビルド直後に15MB上限のバンドルサイズチェックを追加。`npm run check-bundle-size`としてpackage.jsonにも登録。単体テスト3件追加、実ビルド出力(4.89MB)で動作確認済み）
...
## Definition of Done
- [x] 全BDDシナリオが自動テストとして実装されパスする（M20〜M26全項目完了）
- [x] テストカバレッジが基準を満たす（M24: cspDomains.test.ts、M26: check-bundle-size.test.ts、M21: 既存8ファイルを更新。全体スイート6987件通過）
- [ ] コードレビュー完了
- [x] リファクタリング完了（グリーン後）
- [x] i18nメッセージが日英両言語で整合している（M22で対応済み）
