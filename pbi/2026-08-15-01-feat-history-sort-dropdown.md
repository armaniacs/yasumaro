...
- [ ] 「関連度順」は検索クエリが存在する間のみ選択肢に表示され、検索開始時のデフォルトになる
...
- [ ] タグフィルタ・日付フィルタと併用してもソートが正しく適用される
...
- **relevance と検索クエリの整合性**: 「関連度順」はFTS5の `rank` に依存するため、検索クエリが空の状態でrelevanceのままにすると意味を持たない。`search`アクションのリデューサーで検索クエリが空になった際に自動的に`created_at DESC`へフォールバックさせるルールを忘れないこと（`sqliteHistoryPanelState.ts`のテストで担保）
