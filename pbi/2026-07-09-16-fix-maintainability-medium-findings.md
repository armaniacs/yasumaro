...
## 受け入れ基準
- [ ] M15: `dashboard.ts` の3つのMarkdownエクスポート関数（745/823/951付近）が1つのパラメータ化関数に統合され、既存の呼び出し元は変更なく動作する
- [ ] M16: `sqlite.ts` と `opfsWorker.ts` の共通CRUDロジックがStrategyパターン（または共通インターフェース+バックエンド固有実装）として抽出されている
- [ ] M18: `opfsWorker.ts` のメッセージハンドラのinitガードがswitch文前の1箇所に統一されている
- [ ] M19: `migrationService.ts` のtry/catchと進捗追跡ロジックが共通のバッチ処理ループ関数に統合されている
