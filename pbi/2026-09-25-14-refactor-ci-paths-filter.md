...
### 統合 dry-run で発見したテストの欠陥（PR #164 で修正）

`feature/navigation-trail` を main に統合する前提として、main の分類済み `ci.yml`
と本テストを 6 ジョブ版（`bench-check` あり）结合起来で走らせたところ
**6 件落ちた**。原因は PR #162 で main に移植した際の機械的な置換が 2 つの
欠陥を持ち込んだこと。

1. 条件付きタプル `[['bench-check', 'bench']]` の先頭要素が置換で削除され
   `[['bench']]` になっていた。main には `bench-check` が無いので spread が空に
   なり露見しない。
2. 振る舞い行列の期待値から `bench-check` を外したままにしてあり、
   bench-check が存在する側では実際の結果と一致しない。

**「テストは `bench-check` の有無に追随するのでそのまま通ります」と伝えた
予測は誤りだった**。統合前に dry-run で確かめたことで main 側の 1 件に
収まった。修正は PR #164。
