...
## 受け入れ基準
- [ ] `RateLimitService` が `Clock` + `StoragePort(session/local)` を注入される
- [ ] `SessionAlarmService` が `AlarmPort` + `Clock` + `StoragePort` を注入される
- [ ] `encryptionSession` からは `authGuard.isLocked()` 1 seam のみを呼ぶ
- [ ] `ChromeAlarmPort`/`FakeAlarmPort` と `InMemoryStoragePort` で NTP skew や二重ロックを純粋テスト可能
