...
- [ ] 置換中イテレーションの取りこぼしを実証する（または否定する）テストを先に書く。実在が否定された場合は本 PBI を記録つきでクローズする
- [ ] 取りこぼしを実在する場合、置換の適用を「範囲収集 → 1パス適用」に統一し、exec ループ中の文字列再代入をなくす
- [ ] マッチ件数の fail-open 上限（例: 1,000件。超過分は保持し検出を打ち切る）を導入し、上限超過時の挙動を文書化する
- [ ] 制御文字除去の1文字ずつループを regex ベースに置き換える（出力は現行と bit 等価）
- [ ] 既存の promptSanitizer テストスイートが期待値変更なしで green
