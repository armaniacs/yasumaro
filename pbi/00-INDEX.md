...
### 2026-08-09〜10 セッションでアーカイブ済み（2件）
...
| 2026-08-09-22 | 単純委譲パネル9件の宣言表化 | **3番目**（20・21は完了） |
| 2026-08-09-23 | SQLite トランスポート層の削減（Epic） | **4番目**・21完了により着手可 |
...
21（候補03・変更系Result）    ← 19の続き。✅ 完了（アーカイブ済み）
  ↓
22（候補04・浅いパネル）      ← 独立。小さいので箸休めに良い。次はこれ
  ↓
23（候補02・トランスポート）  ← 21が前提（完了済み）。Epic。Phase1だけでも可
  ↓
24（候補05・逆依存）          ← 22の後が望ましい。PBI-09の後継
```

22〜24 の実装計画は `dev-docs/plans/2026-08-09-pbi{22..24}-*-plan.md`。
20・21 の実装計画は完了に伴い `dev-docs/archived/plans/` へ移動済み。
