...
### 2026-08-09 セッション（続き）でアーカイブ済み（1件）
...
| 2026-08-09-21 | SQLite 変更系の Result 貫通・lastError 削除 | **2番目**（1番目の2026-08-09-20は完了） |
...
20（候補01・ルール宣言）      ← 18の続き。✅ 完了（アーカイブ済み）
  ↓
21（候補03・変更系Result）    ← 19の続き。19完了済みが前提。次はこれ
  ↓
...
21〜24 の実装計画は `dev-docs/plans/2026-08-09-pbi{21..24}-*-plan.md`。
20 の実装計画は完了に伴い `dev-docs/archived/plans/` へ移動済み。
