...
## 受け入れ基準
- [ ] M7: Offscreen Documentへの操作が何らかのキューまたは並行制御機構を経由する（未実装: sqliteClient.tsにキュー機構なし）
- [ ] M8: `auditLog.ts` と `reviewSummaryGenerator.ts` がmodule-levelでSqliteClient/AIClientを生成せず、DI経由で受け取る設計に変更されている（未実装）
- [ ] M9: `LEGACY_DUAL_WRITE_ENABLED` 設定キーが追加され、falseの場合chrome.storageへの二重書き込みがスキップされる（未実装: 設定キー自体が存在しない）
...
- [ ] M12: Offscreen Documentへの依存呼び出しにkeepAliveまたはバッチ化の仕組みが導入されている（未実装）
- [ ] M13: クエリ結果取得にLIMIT強制上限が適用される（未実装: LIMIT/OFFSETはあるが強制上限ロジックなし）
- [ ] M14: SQLite利用不可期間のデータ欠落に対する整合性チェックまたは保留メカニズムが実装されている（一部実装: RecordingPipeline.tsで警告ログのみ、保留メカニズムなし）
