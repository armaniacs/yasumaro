...
- 通常版 Firefox への実インストールと 同意→記録→検索 の smoke を**完了**（ユーザー報告）。
  実機でしか確認できない項目なので、確認者の報告を根拠として充足とした。

### 残置（ユーザー作業）

1. 6.9.18 以降を AMO へ提出するかどうかの判断（現在は 6.9.17 で公開中、main は 6.9.26）
2. リスティングのスクリーンショット等 AMO リソースの差し替えは任意
...
- [x] AMO で署名・公開されている
- [x] 通常版 Firefox に AMO からインストールして同意→記録→検索が動作する（2026-09-26 ユーザー報告。実機 smoke 完了）
...
- [x] 全BDDシナリオが完了している（公開・実機 smoke とも完了）
