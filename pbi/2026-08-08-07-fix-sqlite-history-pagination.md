...
### 調査結果（2026-08-08）: 両者は等価ではない → 折衷案を採用

| | クライアント側 `filterRowsByTag` | サーバ側 `tagFilter` |
|---|---|---|
| 実装 | `tags.split(',').some(t => t.trim().includes(tagFilter))` | FTS5 `tags MATCH '"#<cleanTag>"'`（`opfsWorker.ts:284-296`） |
| 方式 | 単純な部分一致 | trigram インデックスへの MATCH |
| 演算子除去 | しない | `["'*^~:()+\-\\]` と `OR/AND/NOT/NEAR` を除去 |
| 短いタグ | 1文字でも一致する | **trigram は3文字以上が必要**。`handleSearch` には LIKE フォールバックがある（`opfsWorker.ts:661-666`）が、**`tagFilter` 経路には無い** |

**結論**: サーバ側 `tagFilter` に丸ごと移すと、2文字以下のタグが**無言で0件になる**回帰が発生する。タグは日本語短語（例: 「AI」「本」）が現実的にありうるため、これは受け入れられない。

**採用する方針（折衷案）**:

- `tagFilter` **なし**の場合 → サーバ側ページング（`limit: PAGE_SIZE, offset: page * PAGE_SIZE`）。これで1000件問題は解消する
- `tagFilter` **あり**の場合 → クライアント側フィルタを維持。ただし取得上限を `MAX_QUERY_LIMIT` 寄りに引き上げ、上限到達時はその旨をユーザーに示せるようにする

タグ絞り込みのサーバ側移行は、`tagFilter` 経路に LIKE フォールバックを実装してから別PBIで行う。
...
- [x] **先に**ページング境界のテストを追加し、現状の欠陥を再現（1000件超で空になることを確認）
- [x] サーバ側 `tagFilter` とクライアント側 `filterRowsByTag` のロジック差異を比較・記録（上記「調査結果」参照。等価でないため折衷案を採用）
- [x] `queryLogs` に `limit: PAGE_SIZE` / `offset: page * PAGE_SIZE` を渡す（tagFilter 無しの場合）
- [x] クライアント側の `slice` を削除（サーバ側ページングと整合）
- [x] タグフォールバック機能の既存テストがパスする
- [x] 追加したページング境界テストがパスする
- [x] `npm run validate` が成功する（7435 tests pass）

### 実装結果（2026-08-08）

- `tagFilter` 無し → `limit: PAGE_SIZE, offset: page * PAGE_SIZE` でサーバ側ページング。クライアント側 slice を削除
- `tagFilter` 有り → `TAG_FILTER_FETCH_LIMIT = 5000` で取得しクライアント側フィルタを維持（FTS5 trigram の3文字制限を回避するため意図的）
- テスト 7431 → 7435（+4）

### テストが本当に欠陥を捕まえることの確認

回帰テストを書いただけでは不十分なため、**修正後に意図的にバグを再導入して落ちることを確認した**。

最初に書いたテストは `offset: 0`（1ページ目）でしか検証しておらず、`slice(0, 20)` が恒等変換になるため**バグを再導入しても素通りした**。ページ送りボタンを実際にクリックして2ページ目へ遷移する形に修正したところ、バグ再導入時に「DBが20件返しているのに `No records found.` と表示される」という実際のユーザー症状を再現して失敗することを確認済み。
