...
## 受け入れ基準
- [ ] ダッシュボードに閲覧時間分析の新規パネルが表示される
- [ ] 期間フィルタで 今日 / 7日 / 30日 / 90日 / 全期間 / カスタム期間を選択できる
- [ ] ドメイン別ランキングに合計時間・平均時間・レコード件数が表示される
- [ ] タグ別ランキングに合計時間・平均時間・レコード件数が表示される
- [ ] 時間表示が人間可読な単位（分・時間）で表示される
- [ ] visit_duration が null の行を集計から除外し、未計測率（除外率）が表示される
- [ ] 期間内0件・全件未計測・上位N件切り捨ての各境界ケースが仕様通りに表示される
- [ ] 表示文言が日本語・英語の両ロケールで提供される
