...
## 受け入れ基準
- [ ] fetch 前に decidePrivacy({force, whitelisted, isPrivate: false, autoSaveBehavior, requireConfirmation}) を呼び allow なら getPrivacyInfoWithCache をスキップする
- [ ] fetch 後の呼び出しが isPrivate マトリクスのみを担う
- [ ] pending 組み立てが pendingStorage.ts の純粋 buildPendingPage(input, clock) に抽出されている
- [ ] buildPendingPage が許可リストと TTL の既存 SSOT を参照する
- [ ] step が I/O のみを担い組み立てロジックを持たない
- [ ] ログ文言と return context 形状が変更前と byte 等価である
- [ ] deniedBy マトリクスに変更がない
