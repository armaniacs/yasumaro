...
なお、OPFS・IndexedDBともにブラウザのストレージサンドボックス内に保存されており、OS上の絶対ファイルパスをWeb APIから取得することはできません（ブラウザの仕様による意図的な制限です）。診断パネルに表示される「OPFS経路」「IDB経路」の値は、取得可能な範囲で最も具体的なブラウザ内部の識別情報です。
...
すでにデータが保存されている（「保存データ件数」が0でない）にもかかわらず「確認中」が続く場合、OPFS移行処理を実行する専用Worker（Worker()コンストラクタで生成されるスレッド）からchrome.storage.localへのアクセスが制限されている可能性があります。この場合も保存済みデータには影響しません。診断パネルの当該行にヒントが表示されます。
