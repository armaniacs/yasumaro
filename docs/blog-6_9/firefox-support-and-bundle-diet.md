...
- `renderChunk` で剥がした結果、**firefox zip は 10.4MB → 2.76MB（-74%）**。chrome の zip はバイト単位で変化なし
...
現時点（v6.9.10）で Firefox でも動くものは次のとおりです。
...
インストールは GitHub Releases の `yasumaro-6.9.10-firefox.zip` からです。CI では `firefox-storage` ジョブが VFS probe・実 dist worker smoke・PII WASM probe を毎回回していて、6.9.2 には送信者検証の回帰 hotfix も出しています。「実験的」と書きましたが、この記事を書く時点では記録から検索まで一通り使える状態まで安定化しています。
...
chrome がバイト単位で不変なのが個人的に気に入っている点です。この変更が Firefox ビルドだけに触れたことの証明になります。
...
- Firefox では旧 wa-sqlite 形式 DB からの移行バックアップが動きません。バックアップ経路が data: インラインの wasm に依存しているためで、公開アセット化すれば直せます（+1.48MB というサイズとのトレードオフがあります）
- chromium 側には誰も読まない wa-sqlite 本家 plain ビルド（558KB）がまだ同梱されています
- この修正は次回リリース（6.9.11 を想定）でユーザーに届きます

Firefox を使いたいのにサイズが理由で躊躇していた人には、次のリリースからは 2.76MB で待っています。
