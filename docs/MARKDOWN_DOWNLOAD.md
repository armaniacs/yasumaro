...
| モード | 説明 |
|-------|------|
| **自動書き出し** | ページが記録されるたびに、その日の Markdown ファイルが自動ダウンロードされます |
| **手動エクスポート** | 開始日と終了日を指定して、SQLite に保存された既存の履歴を Markdown に変換します |
...
自動書き出しは以下の条件をすべて満たした場合にのみ発火します:
...
- これは仕様です。每次記録時に完全な日次ファイルが再ダウンロードされます
...
| Mode | Description |
|------|-------------|
| **Auto Export** | Each time a page is recorded, that day's Markdown file is automatically downloaded |
| **Manual Export** | Specify a date range to convert existing history from SQLite to Markdown |
...
### Auto Export Trigger Conditions

Auto export only fires when ALL of the following are met:
...
- This is by design. Each recording re-downloads the complete daily file
