...
過去のインシデントから、外部API連携の信頼性を確保するための6つの設計パターンを定義する。以下の「インシデントからの教訓」には、かつて CI で利用していた Chrome Web Store API（現在は配布終了により未使用）で発生した事例も含むが、いずれの教訓も現行の外部API連携にそのまま適用される。
