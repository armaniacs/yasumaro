...
## 受け入れ基準
- [x] `StorageKeys.YASUMARO_MIGRATION_STATUS` キーでマイグレーション状態を管理する
- [x] 移行対象: 既存の `savedUrlsWithTimestamps` chrome.storage.local → SQLite browsing_logs
- [x] バッチ処理（100件ずつ）で大量データ移行時のメモリ圧迫を防ぐ
- [x] Service Worker再起動をまたいでも移行が継続できる（進捗を保存する）
- [ ] 移行完了後にポップアップ経由でユーザーへ通知する（未実装）
- [ ] 移行済みデータ件数をダッシュボードで確認できる（未実装）
...
## Definition of Done
- [x] 全BDDシナリオが自動テストとしてパスする（6 tests）
- [x] `npm run type-check` 通過
- [x] コードレビュー完了
- [x] 既存機能（Obsidian連携・AI要約）がリグレッションしない
- [ ] 500件のデータ移行が30秒以内に完了する（性能要件）（⚠️ 実機E2E未検証）
