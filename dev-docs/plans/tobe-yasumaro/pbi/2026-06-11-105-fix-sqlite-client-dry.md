...
## 受け入れ基準
- [ ] `call<T>()`プライベートヘルパーメソッドを導入
- [ ] 全11メソッドが`call<T>()`を使用するように変更
- [ ] 型推論が正しく動作することを確認
- [ ] 既存テストが全てパスすることを確認
...
### 実装手順
1. `call<T>()`プライベートヘルパーを定義
2. 各メソッドを`call<T>()`を使用するように変更
3. 型推論の確認
4. 既存テストの実行

### 落とし穴
- `call<T>()`の`transform`関数の型シグネチャが複雑になる可能性
- `msgOffscreen`の戻り値型が`unknown`のため、型アサーションが必要
- エラーメッセージのフォーマット統一
