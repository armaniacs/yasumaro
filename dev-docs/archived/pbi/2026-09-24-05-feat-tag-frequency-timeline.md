...
## 受け入れ基準
- [x] 期間内で出現数上位のタグ（既定10、表示数は設定変更可能）の週別/月別の記録数が積み上げ折れ線または積み上げ面グラフで表示される
- [x] 粒度切替（週/月）が動作し、切替のたびに再集計される
- [x] 週の開始はローカル時刻の日曜であり、 UI 上に明記される
- [x] 共有の期間フィルタ部品の選択範囲に連動して集計範囲が変わる
- [x] グラフと同値の数値テーブルが提供され、キーボードのみで粒度・表示数・テーブルの参照が操作できる
- [x] 上位N件から漏れたタグは「その他」系列に合算され、凡例とテーブルに明示される
- [x] タグ0件・端数週・取得上限到達の各状態が空状態または注記として表示される
- [x] 日英両ロケールで文言が表示され、コントラスト等の外観がデザイントークンに準拠する
...
- 実装: `src/dashboard/tagFrequencyTimeline.ts`（純粋集計・週開始日曜は暦日演算で DST ズレ回避・端数週は1バケット・実在範囲駆動で空週なし・`otherCount` を実タグ名 "other" と構造的に分離・1タグ1計上）、`src/dashboard/panels/asyncData/tagFrequencyTimelinePanel.ts`（手描き SVG 積み上げ・数値テーブル・キーボード操作可・凡例タグから履歴遷移）、`MAX_TAG_TIMELINE_ROWS` 追加、配線（catalog/factories/index.html/locales/panelCatalog pinned 21→22）
- 逸脱（記録済み）: `narrowEntriesToTopTagsHybrid` を使わず単一 O(n) パスで全タグ計数→ランキング（先に絞ると「その他」合算の件数が失われるため）。粒度・top-N 切替はキャッシュ行の再集計（再取得なし）、期間変更のみ明示適用
- 検証: type-check PASS / 対象 62 tests green / lint 0 errors / 全体 13,694 tests green / build PASS
