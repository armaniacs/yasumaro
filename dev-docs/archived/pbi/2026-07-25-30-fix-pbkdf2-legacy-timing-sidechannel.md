...
**作成日**: 2026-07-25
**再確認日**: 2026-07-26（直近コミット `8d1891e` の内容と照合し、記述が現状と一致していることを確認済み。変更なし）
**完了日**: 2026-07-26
**優先度**: High
**見積もり**: 🟡中（2pt目安）
...
`src/utils/crypto.ts` の `verifyPasswordWithPBKDF2` レガシーパス（377-391行）を、新iteration一致時に
早期リターンする構造から、新旧両方のハッシュを常に計算してから判定する構造に変更した。
`constantTimeCompare` の呼び出し自体も両方のケースで必ず実行されるようにし、`if (newMatches)` /
`if (legacyMatches)` の分岐は計算後の結果参照のみで、計算タイミングには影響しない。

`crypto.test.ts` に3件のテストを追加（新iteration一致→`needsRehash: false`、旧iteration一致→
`needsRehash: true`、新旧不一致→`isValid: false`）。既存の定数時間比較テストを含め全76件パス、
型チェック・全テストスイート（7362件）も回帰なし。
