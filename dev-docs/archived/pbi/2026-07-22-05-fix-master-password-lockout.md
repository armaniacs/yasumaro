...
- [ ] **5 Whys対策**: `rateLimiter.ts` にキー設計のコメントを追加し、「UI経路ごとに異なるキーを使用する（popup/dashboard は分離）」ことを明記する
...
```
RateLimiter [community=192/212]
    ├─ checkRateLimit() ← unlockWithPassword() [encryptionSession.ts]
    ├─ recordFailedAttempt() ← unlockWithPassword() [encryptionSession.ts]
    ├─ checkRateLimit() ← authenticatePassword() [dashboard/masterPassword.ts]
    ├─ recordFailedAttempt() ← authenticatePassword() [dashboard/masterPassword.ts]
    └─ masterPasswordUi.ts [community=38] ← 既存のレートリミット実装

encryptionSession.ts [community=28] ← crypto.ts [deriveKey, hashPasswordWithPBKDF2]
masterPassword.ts [dashboard, community=48]
```
**重要な発見**: 
- `rateLimiter.ts` は `chrome.storage.session` に状態を保存 (`SessionStore` community=167 と連携)
- `withOptimisticLock()` (`optimisticLock.ts`) がプロジェクトに存在 — レートリミットのアトミック化に最適なパターンが既に利用可能
- ポップアップ側 (`masterPasswordUi.ts`) は既にレートリミットを実装済み。ダッシュボード側との差分は「呼び出しの欠落」のみ。

### なぜなぜ分析（2026-07-22）
**仮定**: 「`rateLimiter.ts` のキーは呼び出し元ごとに分離されている」
- Why 1: なぜ分離が必要か → ポップアップとダッシュボードが同じカウンタを共有すると、一方の操作が他方のロックアウトを引き起こすため
- Why 2: なぜ共有が問題か → ユーザーがポップアップで5回失敗するとダッシュボードでもロックアウトされる（逆も然り）
- Why 3: なぜ分離設計になったか → `masterPasswordUi.ts` 実装時に「UIごとに独立したカウンタ」が選ばれたため
- Why 4: なぜ独立が選ばれたか → 同一ユーザーが複数の認証UIを持つ構成が「当然」と設計当时は認識されていたため
- Why 5: なぜ認識されていたか → ポップアップとダッシュボードが別々のタイミングで実装され、統合時にこの認識がドキュメント化されなかったため
- **根本原因**: 認証UIの複数経路が「独立したカウンタを持つ」という設計判断が暗黙的で、ドキュメント化されていない
- **対策**: `rateLimiter.ts` にキー設計のコメントを追加し、「UI経路ごとに異なるキーを使用する」ことを明記する
