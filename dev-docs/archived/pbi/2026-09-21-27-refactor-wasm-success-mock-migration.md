...
- [x] `src/background/pipeline/__tests__/piiSanitizeHybrid.wasm-success.test.ts` が `createNodeWasmInit` を使い、独自の `fileURLToPath` と `readFile` と `initPromise` メモを持たない
- [x] `src/dashboard/__tests__/tagCooccurrenceHybrid.wasm-success.test.ts` が `createNodeWasmInit` を使い、独自の `fileURLToPath` と `readFile` と `initPromise` メモを持たない
- [x] 両 mock の `return` 形状が移行前と同一である（`sanitizePii` 直接 import 維持、`vi.importActual` spread 維持）
- [x] `grep -rn "module_or_path: bytes" src/background src/dashboard` で呼び出し側の直書きがゼロである
- [x] `wasm/tag-cooccur/src/cooccur.rs` の陳腐 doc 1行と `wasm/js-strings/src/jsstring.rs` の陳腐 doc 1行が委譲後の実態に更新される
