...
- [ ] **5 Whys対策**: VULN-003/016の修正時に「Mutex vs optimisticLock 使用基準」が ADR に記録され、今後同様の競合状態修正で統一されたパターンが適用される
...
```
Mutex [community=214] ← プロジェクト全体で複数使用、alarm handlerでは未使用
flushBufferedExports() → service-worker.ts [community=4]
checkDuplicateStep.ts → saveMetadataStep.ts [既存パイプライン]
SessionStore [community=167] ← chrome.storage.session 状態管理
```
**重要な発見**: 
- `Mutex` クラス (community=214) はプロジェクト全体の並行制御パターンとして確立されている。alarm handler で未使用なのみ。
- `withOptimisticLock()` (`optimisticLock.ts`) がプロジェクトに存在 — VULN-016 (offscreen SQLite race) の修正に最適なパターンが既に利用可能。
- 「Concurrency Control Infrastructure」ハイパーエッジが検出: `optimistic_lock_pattern`, `mutex_pattern`, `rate_limiter_module` が関連。

### なぜなぜ分析（2026-07-22）
**仮定**: 「Mutex と optimisticLock のどちらを使うかは実装者が選べる」
- Why 1: なぜ選択肢があるか → 両方がプロジェクトに存在し、どちらも並行制御の実績があるため
- Why 2: なぜ両方存在するか → 異なるPBIで異なるパターンが導入された歴史的経緯があるため
- Why 3: なぜ統一されなかったか → 「最適なツールを選ぶ」という柔軟性が重視され、標準化が後回しにされたため
- Why 4: なぜ標準化が後回しになったか → 各機能が独立して実装され、横断的なアーキテクチャ検証のタイミングがなかったため
- Why 5: なぜ横断検証がなかったか → 機能追加のレビュープロセスが「個別PR」中心で、複数PRに跨るパターン統一を担保する仕組みがないため
- **根本原因**: 横断的なアーキテクチャ標準化を担保する仕組みの不在
- **対策**: 本PBIの実装時に「VULN-003/016はMutex、VULN-014はoptimisticLock」という使用基準をADRに記録する
