...
## 受け入れ基準
- [ ] バイト欠損時に行自体を消さず理由ラベルを表示する
- [ ] AIなし記録と旧記録と空ページを文言で区別する
- [ ] AI Summary Cleansing行の欠損時も理由表示の対象にする
- [ ] プログレスバーは従来通り欠損時は非表示のままとする。理由表示の対象外である
- [ ] 日英の両localeで文言が表示される
- [ ] 既存の正常エントリの表示が変わらない。短文と過剰削除のfallback経路は数値表示のままにする
- [ ] XSS対策として理由文言と数値のescapeを維持する
- [ ] 理由ラベルは既存の診断行と同じ文字サイズとコントラストで読み取れる
...
- [ ] リファクタリング完了（グリーン後）
- [ ] ロールバック手段の検討（表示層のみの変更のため、理由行の追加分岐をfeature flagなしでrevertできることをPR説明に記載する。locale key追加は残しても無害である）
- [ ] ドキュメント更新済み（履歴パネル関連の説明に理由表示の文言と4分類を追記する）
- [ ] pbi/00-INDEX.mdに本PBIの行を追加する
