...
- [ ] **5 Whys対策**: PBI-01への依存を削減するため、PBI-01で作成される URL-safe サニタイズ関数が PBI A 完了前でも独立してテスト・実装できる設計になっている
...
```
exportLogsService.ts → dashboardSqliteService.ts → dashboardSqliteHandlers-append.test.ts → obsidianFormatter.ts → sanitizeForObsidian()
```
**重要な発見**: `exportLogsService.ts` と `obsidianFormatter.ts` はテストインフラ (`dashboardSqliteHandlers-append.test.ts`) を介して `sanitizeForObsidian()` に接続されているが、本番コードでは直接サニタイズを適用していない。`dashboard.ts` は `sanitizeForObsidian` をインポート済みだが `formatEntryToMarkdown` では未使用。

「SyncTarget Interface → ObsidianSyncService」の関連が graphify で検出済み。GistSyncTarget/ReviewSummaryGenerator も同じ抽象の下流。

### なぜなぜ分析（2026-07-22）
**仮定**: 「PBI-01完了後に着手すれば効率的」
- Why 1: なぜ PBI-01 に依存するか → `sanitizeForObsidian()` の scheme-agnostic 化が前提
- Why 2: なぜ scheme-agnostic 化が前提か → `url` のサニタイズに `sanitizeForObsidian()` を流用する設計のため
- Why 3: なぜ流用するか → 新規サニタイズ関数を作らず、既存の検証済み関数を拡張する方が安全と考えたため
- Why 4: なぜ既存関数の拡張を選んだか → 車輪の再発明を避け、テストカバレッジを維持したいため
- Why 5: なぜ車輪の再発明を避けたいか → プロジェクトに「既存資産の再利用」という暗黙の設計原則があるため
- **根本原因**: PBI-01とPBI-02の依存関係は技術的なものではなく、開発者の再利用意思に起因する
- **対策**: PBI-01の新規 URL-safe サニタイズ関数を汎用的に設計し、PBI-02 でも独立して実装できるようにする
