...
- [x] セッション内のどれかの記録に `nav_source_url` があれば、そのセッションの展開部を入れ子の `<ul>` によるツリーで描画する。1 件もなければ 02 と同じ平坦な一覧で描画する
- [x] 親は「同じセッション内で自分より前にあり、URL が自分の流入元と一致する記録のうち最も新しいもの」とする。URL の比較は `normalizeNavUrl` で正規化してから行う
- [x] 先頭の記録に `search_query` があれば、ツリーの前に「検索: …」を表示する
- [x] 検索→解決指標の算出規則
  ...
  - 並び順: 平均ページ数の降順
- [x] 平均ページ数は小数 1 桁、平均分は整数で表示する
- [x] 遷移記録が無効なら、指標の表を隠して案内文を表示する
- [x] ガイド（ja / en）に次の 3 点を書く: 解決ページの定義、指標の限界（同日の再訪はスキップされ、記録にはゲートもあるため、記録されない経路がある）、有効にする手順
