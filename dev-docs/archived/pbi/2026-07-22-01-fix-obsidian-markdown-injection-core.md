...
- [ ] **5 Whys対策**: プロジェクトに「全markdown出力経路は必ず `sanitizeForObsidian()` を通す」という統制ルールが ADR/ガードレールドキュメントに明記されている
...
```
obsidianSyncService.ts → ObsidianClient
    ↓ テスト経路 (dashboardSqliteHandlers-append.test.ts)
    → obsidianFormatter.ts → sanitizeForObsidian()

gistSyncTarget.ts → test → recordCurrentPage.ts → markdownFormatter.ts → sanitizeForObsidian()
```
**重要な発見**: テストインフラは既に `sanitizeForObsidian()` を使用する正しいパスを認識しているが、本番コード4ファイルはこのパスをバイパスして未サニタイズのmarkdownを生成している。修正は本番コードに既存パターンを適用するのみ。

プロジェクトハイパーエッジ「VulnHunter 2026-07-21 Fix Batch」が検出済み。本PBIは確立された修正シリーズの一部。

### なぜなぜ分析（2026-07-22）
**仮定**: 「既存テストが壊れなければ `sanitizeAllMarkdownLinks` に移行できる」
- Why 1: なぜ壊れると困るか → 既存テストが「エスケープしない」相対URLパターンを検証しているため
- Why 2: なぜ相対URLをエスケープしないか → `sanitizeMarkdownLinks` が http(s) のみを対象としているため
- Why 3: なぜ http(s) のみか → 元々の脅威モデルが「外部リンクのスキーム偽装」に限定されていたため
- Why 4: なぜ脅威モデルが限定的だったか → `sanitizeForObsidian()` がパイプライン経路のみで使用され、新規ファイルでの適用ルールが明文化されていなかったため
- Why 5: なぜルールが明文化されていなかったか → セキュリティ要件がコードコメントに分散し、「全markdown出力経路は必ずサニタイズする」という統制ポイントが存在しないため
- **根本原因**: 全markdown出力経路へのサニタイズ適用ルールの不在
- **対策**: ADRに「Markdown出力ルール」を追加し、新規ファイル作成時にサニタイザ適用を義務付ける
