...
- [x] `searchInput` が `{ path: 'fts'; ftsQuery: string } | { path: 'like'; rawTerm: string }` の判別共用体になり `path` field と統合されている
- [x] `buildLikePattern` の適用が union 消費点の 1 箇所に限定されている
- [x] FTS path に生 term を渡す誤用が型検査で拒否される
- [x] SQL 文・params 順序・rows・total が変更前と byte 等価である（挙動不変）
- [x] 既存の pin スナップショットが無変更で green である
