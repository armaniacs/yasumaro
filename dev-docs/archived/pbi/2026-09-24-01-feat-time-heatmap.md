...
## 受け入れ基準
- [x] ダッシュボードに新規パネルとして曜日7×時間帯24の記録数ヒートマップが表示される
- [x] 集計対象は直近12ヶ月の rolling window に固定され、期間ピッカーは表示されない
- [x] 色の強度は件数に比例し、強度スケールは design tokens の定義に従う
- [x] ホバーまたはフォーカスで曜日・時間帯・件数がテキスト表示される
- [x] ヒートマップと同等の内容を持つ数値テーブルが提供され、キーボードのみで閲覧できる
- [x] 記録0件の場合は空状態メッセージが表示される
- [x] 10000行の取得上限到達時は上限に関する注意表示が出る
- [x] 日英両言語で表示文言が正しく切り替わる
