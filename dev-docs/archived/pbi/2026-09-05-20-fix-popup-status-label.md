...
## 受け入れ基準
- [x] `statusDomainIcon`・`statusPrivacyIcon` の隣に可視テキストラベルが表示される（色のみの伝達を解消）
- [x] 既存の `aria-label`（記録可能・ブロック中・プライベート検出等）が維持され、スクリーンリーダーで判別できる
- [x] 日英両ロケールでラベル文字列が `getMessage` 経由で解決される（ハードコードなし）
- [x] popup関連の既存テストがパスする（必要に応じて期待値を更新）
...
- [x] コードレビュー完了
- [x] ドキュメント更新済み（ACCESSIBILITY.md の該当箇所があれば）

## 実装メモ（2026-09-05・branch 0905c）
- 完了（commit `65dd4114`、SDD サブエージェント実装）。ステータスサマリーアイコンに可視テキストラベル追加（data-i18n・en/ja）。副次修正: 欠落キー `statusPublicPage` を追加（公開状態の aria-label 空文字解消）。check-i18n PASS。実機目視は手動確認項目として残置。
