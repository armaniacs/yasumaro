...
**スクリーンショット説明**: ダッシュボードの初期設定タブ。左サイドバーに「初期設定」「ドメインフィルター」「AIプロンプト」「プライバシー」「記録履歴」「ドメイン検索」「診断」「エクスポート/インポート」のメニューが並び、右側に設定フォームが表示されている。
...
**スクリーンショット説明**: AIプロンプトタブ。保存済みプロンプトの一覧が表示され、各プロンプトに「編集」「複製」「削除」ボタンが並んでいる。デフォルトプロンプトには「編集不可」のラベルが付いている。

デフォルトのプロンプトは編集・削除できません。まず「複製」ボタンで手元にコピーを作り、そこから変えていく設計にしました。いつでも初期状態に戻せるようにするためです。

**スクリーンショット説明**: デフォルトプロンプトの詳細表示。プロンプト編集エリアの上部に「複製」ボタンが強調表示され、デフォルトプロンプトは複製してから編集する旨の案内テキストが表示されている。
