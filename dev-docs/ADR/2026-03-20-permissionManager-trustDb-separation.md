...
- 機能:
  ...
  - ユーザー拒否ドメインの記録（deniedDomains）
  - `isHostPermitted`は`chrome.permissions.contains`への単純委譲。プリセットドメイン自体の判定ロジックは持たない（プリセットドメインはmanifest生成時にhost_permissionsへ静的に反映される）
...
> **注記**: その後のパイプライン化により、統合先は`recordingLogic.ts`から`src/background/pipeline/steps/checkPermissionStep.ts`（Permissionチェック）・`checkTrustDomainStep.ts`（Trustチェック）に分離されました（`RecordingPipeline.ts`経由）。順序（Permission→Trust）は変わりません。

recordingLogic.ts内で以下の順序で使用すること（決定当時の設計。現在の実装先は上記注記参照）：
```typescript
