...
> **注記**: 以下は決定当時（2026-02-22）の実装スニペットです。その後 `storage.ts` は複数モジュールに分割され、`DEFAULT_SETTINGS` は `src/utils/storage/defaults.ts` に、`buildAllowedUrls` は `src/utils/allowedUrls.ts` に移動しています（値自体は現在も `protocol: 'https'` / `port: '27124'` のまま一致）。
...
- ユーザー向け移行ガイド・レビュードキュメントは当時作成されたが、その後削除・未保存のため現存しない
