...
`src/utils/retryHelper.ts` の `ServiceWorkerResponse` インターフェースにおける `maskedItems` フィールドは、当初 `any[]` 型として意図的に保持されていましたが、その後 `unknown[]` 型に更新されています（プロジェクト規約「any/unknown型を使わない」に伴う型安全性向上）。以下は当時の設計判断の記録です：
