...
<p align="center">
  ...
  <a href="https://github.com/armaniacs/yasumaro/releases/latest" target="_blank" rel="noopener">
    <img src="https://img.shields.io/badge/GitHub_Releases-Download-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub Releases" />
  </a>
...
#### 方法2: GitHub Releases の zip を読み込む（Chrome / Brave など）
...
ドメインリストではワイルドカードも使用できます（例: `*.example.com`）。「サブドメインもマッチさせる」トグルをONにすると、`example.com` の登録が `sub.example.com` 等のサブドメインにも一致します（デフォルトOFF）。「現在のページドメインを追加」ボタンで簡単にドメインを追加できます。
...
- 📊 **Data Analytics Panels**: Analyze your recorded history in the dashboard — a day×hour heatmap, domain/URL top-N tables, tag timelines, a tag co-occurrence pair table, keyword clusters (no tagging required), side-by-side tag cluster comparison, and a revisits & time capsule view that groups what you keep re-researching and copies it out as Markdown. All computed locally against the on-device SQLite database. See [Data Analytics Guide](docs/DATA_ANALYTICS_GUIDE.md).
...
#### Option 2: Load the GitHub Releases zip (Chrome / Brave, etc.)
...
You can use wildcards in the domain list (e.g., `*.example.com`). With the "Match subdomains too" toggle ON, an `example.com` entry also matches subdomains like `sub.example.com` (default OFF). Use the "Add Current Domain" button to easily add domains.
