...
4. AI Summary Cleansing（有効な場合）— 同じクローンに対して実行
   ...
   - SNS/プロモ削除（有効な情况下）— スポンサー製品・トレンド等
...
- **高度クレンジングオプション**:
  ...
  - **推荐セクション削除**（デフォルト: 無効）: おすすめ・関連知見・ランキング等を削除（Amazon、Yahoo!、Game8等）
  - **ページネーション削除**（デフォルト: 無効）: 次へ/前へ・ページ番号を削除
  - **SNS/プロモ削除**（デフォルト: 無効）: スポンサー製品・トレンド等を削除
  - **ポップアップ削除**（デフォルト: 無効）: モーダル・トースト通知・cookie同意バーを削除
...
- **Advanced Cleansing Options**:
  ...
  - **Recommendation sections** (default: disabled): Remove recommended articles, rankings (Amazon, Yahoo!, Game8)
  - **Pagination** (default: disabled): Remove next/prev, page numbers
  - **SNS/Promo** (default: disabled): Remove sponsored products, trends
  - **Popups** (default: disabled): Remove modals, toast notifications, cookie consent bars
