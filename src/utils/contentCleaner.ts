...
/**
 * 新規インストール時の初期値（17語のサブセット）。
 * defaults.ts の CONTENT_STRIP_KEYWORDS 既定値と contentExtractor の
 * フォールバック既定値の唯一の情報源。リセットボタンは下の
 * DEFAULT_KEYWORDS（全量リスト）を復元する。
 */
export const INITIAL_KEYWORDS: readonly string[] = [
