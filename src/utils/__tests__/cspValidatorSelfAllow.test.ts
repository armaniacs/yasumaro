...
describe('CSPValidator - self-allow hardening (PBI 29-19)', () => {
  ...
  describe('汚染設定は条件付き CSP に追加されない', () => {
    it('does not add provider_base_url when it is the metadata endpoint (169.254.169.254)', () => {
    ...
    it('does not add provider_base_url when it is metadata.google.internal', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
    ...
    it('does not add provider_base_url when it is a private IP (10.x/192.168.x/172.16-31.x)', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
    ...
    it('does not add openai_base_url when it is non-https and non-localhost (http://evil.example)', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
    ...
    it('does not add non-http(s) schemes (file:, ftp:)', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
  ...
  describe('正当な設定は引き続き許可される (回帰防止)', () => {
    ...
    it('adds a localhost custom endpoint (Ollama/LM Studio)', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
    ...
    it('adds a 127.0.0.1 custom endpoint', () => {
      CSPValidator.initializeFromSettings({
        conditional_csp_providers: [],
