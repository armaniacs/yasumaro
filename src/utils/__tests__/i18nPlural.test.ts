...
describe('getPluralKey', () => {
    beforeEach(() => {
        (chrome.i18n.getUILanguage as Mock).mockReturnValue('en');
    });
    ...
    it('falls back to base key for Japanese', () => {
        (chrome.i18n.getUILanguage as Mock).mockReturnValue('ja');
    ...
    it('falls back to base key for unknown locales', () => {
        (chrome.i18n.getUILanguage as Mock).mockReturnValue('xx-YY');
        expect(getPluralKey('itemsCount', 1)).toBe('itemsCount');
    });
});
