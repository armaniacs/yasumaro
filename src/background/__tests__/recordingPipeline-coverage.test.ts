...
// Orchestrator mock. `record` delegates to an internal `execute` spy so the
...
vi.mock('../pipeline/RecordingOrchestrator.ts', async () => {
  ...
  const RecordingOrchestrator = vi.fn().mockImplementation(function(this: any) {
    ...
    this.record = async (data: unknown, opts?: { settings?: unknown }) => {
      const settings = opts?.settings ?? await RealRecordingCache.getSettingsWithCache();
...
import { RecordingOrchestrator } from '../pipeline/RecordingOrchestrator.ts';
...
// Re-arms the mocked RecordingOrchestrator's execute() while keeping the
...
function mockPipelineExecute(mockExecute: ReturnType<typeof vi.fn>): void {
  ...
  RecordingOrchestrator.mockImplementation(function(this: any) {
    this.execute = mockExecute;
    this.record = async (data: unknown, opts?: { settings?: unknown }) => {
      const settings = opts?.settings ?? await RecordingCache.getSettingsWithCache();
      return this.execute(data, settings);
    };
  });
}
...
describe('RecordingPipeline - getSavedUrlsWithCache', () => {
  let logic: RecordingOrchestrator;
...
describe('RecordingPipeline - invalidateUrlCache', () => {
  ...
  test('clears urlCache so the next read refetches', async () => {
    // @ts-expect-error - vi.fn() type narrowing
    storageSavedUrls.getSavedUrlsWithTimestamps.mockResolvedValue(new Map([['https://x.com', 1]]));
    await RecordingCache.getSavedUrlsWithCache();
...
describe('RecordingPipeline - normalizeUrlForCache (via getPrivacyInfoWithCache)', () => {
  ...
  test('normalizes URL by removing trailing slash', async () => {
    ...
    RecordingCache.setPrivacyCacheEntry(normalizedKey, privacyInfo);
...
describe('RecordingPipeline - getPrivacyInfoWithCache session storage fallback', () => {
  ...
  test('restores privacy info from chrome.storage.session on cache miss', async () => {
    ...
    expect(RecordingCache.getPrivacyCache()?.get(normalizedUrl)).toBeTruthy();
  });
  ...
  test('handles session storage error gracefully', async () => {
    ...
    const originalGetImpl = (chrome.storage.session.get as Mock).getMockImplementation();
    (chrome.storage.session.get as Mock).mockRejectedValueOnce(new Error('Session error'));

    const result = await RecordingCache.getPrivacyInfoWithCache('https://example.com/page');
    expect(result).toBeNull();

    // Restore
    (chrome.storage.session.get as Mock).mockImplementation(originalGetImpl as any);
  });
  ...
  test('skips session storage fallback when in-memory cache has valid entry', async () => {
    ...
    RecordingCache.setPrivacyCacheEntry(url, cachedInfo);
...
describe('RecordingPipeline - invalidatePrivacyCache', () => {
  ...
  test('clears privacy cache so the next read misses', async () => {
    RecordingCache.setPrivacyCacheEntry('test', { isPrivate: false, timestamp: Date.now() } as never);
...
describe('RecordingPipeline - record (delegates to RecordingPipeline)', () => {
  ...
  test('creates RecordingOrchestrator and calls execute', async () => {
    ...
    expect(RecordingOrchestrator).toHaveBeenCalledTimes(1);
...
describe('RecordingPipeline - settings cache interaction with record', () => {
  ...
  test('record populates settings cache', async () => {
    ...
    // Settings cache was populated by record: a follow-up read hits the
    // cache instead of the repo.
...
describe('RecordingPipeline - static cache state', () => {
  ...
  test('invalidateSettingsCache forces refetch on next read', async () => {
    mockGetAll.mockResolvedValue({ privacy_mode: 'test' });
    await RecordingCache.getSettingsWithCache();
    mockGetAll.mockClear();
  ...
  test('two invalidations still refetch exactly once on next read', async () => {
    mockGetAll.mockResolvedValue({ privacy_mode: 'test' });
    await RecordingCache.getSettingsWithCache();
    mockGetAll.mockClear();
...
describe('RecordingPipeline - edge cases', () => {
  ...
  test('getPrivacyInfoWithCache returns null for expired cache entry', async () => {
    ...
    RecordingCache.setPrivacyCacheEntry(url, expiredInfo);
  ...
  test('getPrivacyInfoWithCache returns fresh cache entry', async () => {
    ...
    RecordingCache.setPrivacyCacheEntry(url, freshInfo);
