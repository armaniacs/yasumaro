...
/**
 * Exhaustive per-type table: `Record<SqliteMessageType, ...>` makes the
 * compiler reject any new message type until it gets an explicit policy
 * decision. A string-keyed partial table silently degraded new types to
 * "never retry" via the `?? RETRY_UNCONFIGURED` fallback, quietly losing the
 * retry-once recovery the M12 fix depends on.
 */
const RETRY_POLICY_TABLE: Record<TransportMessageType, TransportRetryPolicy> = {
  ...
  // Handler-backed but not routed through any wire table (no gateway
  // sender); an explicit decision keeps the table exhaustive.
  SQLITE_SEARCH: RETRY_UNCONFIGURED,
  SQLITE_EXPORT: RETRY_UNCONFIGURED,
};

export const TRANSPORT_RETRY_POLICIES: ReadonlyMap<string, TransportRetryPolicy> = new Map(
  Object.entries(RETRY_POLICY_TABLE),
);
