...
describe('sqliteHistoryPanelController — requestGeneration race guard', () => {
  ...
  it('bumpGenerationOnUnmount causes any still-pending fetch to be discarded', async () => {
...
describe('sqliteHistoryPanelController — pendingInit lifecycle', () => {
  it('activateWithTag sets pendingInit and consumePendingInit returns tagFilter options', async () => {
    const queryHistory = vi.fn().mockResolvedValue({ data: { rows: [], total: 0 } });
    const controller = createSqliteHistoryController({ queryHistory, onStateChange: vi.fn() });

    controller.activateWithTag('AI');
    const opts = controller.consumePendingInit();

    expect(opts).toEqual({ tagFilter: 'AI', tagInitiated: true, limit: 20 });
    expect(controller.getState().activeTagFilter).toBe('AI');
  });

  it('activateWithDomain sets pendingInit and consumePendingInit returns search options', async () => {
    const queryHistory = vi.fn().mockResolvedValue({ data: { rows: [], total: 0 } });
    const controller = createSqliteHistoryController({ queryHistory, onStateChange: vi.fn() });

    controller.activateWithDomain('example.com');
    const opts = controller.consumePendingInit();

    expect(opts).toEqual({ search: 'example.com', limit: 20 });
  });

  it('consumePendingInit returns null and clears pendingInit when nothing was set', () => {
    const controller = createSqliteHistoryController({ onStateChange: vi.fn() });

    expect(controller.consumePendingInit()).toBeNull();
    // Second call still returns null (no leftover state to leak into a later call).
    expect(controller.consumePendingInit()).toBeNull();
  });

  it('consumePendingInit is consumed exactly once (loadData/onActivate race)', () => {
    ...
    controller.activateWithTag('AI');
    expect(controller.consumePendingInit()).not.toBeNull();
    // A second loadData-style consumption must not see the same init again.
    expect(controller.consumePendingInit()).toBeNull();
  });
});
...
describe('sqliteHistoryPanelController — checkFallbackStatus', () => {
  ...
  it('does not throw and does not set fallbackMode when getSqliteStatus rejects', async () => {
    ...
    await expect(controller.checkFallbackStatus()).resolves.toBeUndefined();
