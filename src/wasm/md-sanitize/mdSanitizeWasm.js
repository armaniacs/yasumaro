/* @ts-self-types="./mdSanitizeWasm.d.ts" */
 ...
 * @returns {any}
 */
export function sanitizeBatch(inputs) {
    const ret = wasm.sanitizeBatch(inputs);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function sanitizeBatchAndJoin(inputs, separator) {
    const ptr0 = passStringToWasm0(separator, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.sanitizeBatchAndJoin(inputs, ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @returns {string}
 */
export function sanitizeForObsidian(input) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(input, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.sanitizeForObsidian(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        throw new Error('md-sanitize wasm: module_or_path is required (no default asset URL — see postprocess-wasm-glue.mjs)');
    }
    const imports = __wbg_get_imports();
